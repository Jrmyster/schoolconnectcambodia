...
import { ChevronRight, ChevronDown, MapPin, Briefcase, Star, BookOpen } from "lucide-react";
...
const majors = careersData as Major[];
...
function DesktopView() {
  ...
  const [activeMajorId, setActiveMajorId]   = useState<string>(majors[0].id);
  const [activeCareerIdx, setActiveCareerIdx] = useState<number>(0);

  const activeMajor  = majors.find(m => m.id === activeMajorId) ?? majors[0];
  const activeCareer = activeMajor.careers[activeCareerIdx] ?? activeMajor.careers[0];
          ...
          {majors.map((m) => {
            ...
            return (
              <button
                ...
                className="w-full text-left px-4 py-4 border-b flex items-center gap-3 transition-colors group"
                style={{
                  borderColor: "#EFF6FF",
                  background: active
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className={`text-xs mt-0.5 ${kh ? "" : "font-khmer"}`} style={{ color: "#64748B" }}>
                    {kh ? m.en : m.kh}
                  </p>
                </div>
                <ChevronRight
                  ...
                  style={{ color: active ? "#2563EB" : "#CBD5E1", transform: active ? "translateX(2px)" : "none" }}
                />
              </button>
            );
          })}
          ...
          {activeMajor.careers.map((career, idx) => {
            ...
            return (
              <button
                ...
                className="w-full text-left px-4 py-4 border-b flex items-center gap-3 transition-colors"
                style={{
                  borderColor: "#EFF6FF",
                  background: active
                    ? "linear-gradient(90deg,#EFF6FF,#DBEAFE)"
  ...
  return (
    <div className="lg:hidden rounded-2xl overflow-hidden border" style={{ borderColor: "#BFDBFE" }}>
      {majors.map((major, mi) => {
        ...
        return (
          <div key={major.id} className={mi > 0 ? "border-t" : ""} style={{ borderColor: "#BFDBFE" }}>
            {/* Major header */}
            <button
  ...
  return (
    ...
    >
      ...
      <p className={`mt-4 text-xs text-center text-slate-400 ${kh ? "font-khmer" : ""}`}>
        {kh
          ? "ទិន្នន័យផ្លូវអាជីពត្រូវបានរក្សាទុកក្នុង careers.json — អ្នកអាចបន្ថែម Major ថ្មី។"
          : "Career data lives in careers.json — add new majors anytime without touching this component."}
      </p>
    </section>
  );
}
