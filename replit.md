...
- **Mobile Menu Accessibility**: Hamburger button exposes `aria-expanded` / `aria-controls` with bilingual labels; the mobile menu container has `role="dialog"` and an `aria-label`; pressing **Escape** or navigating to a new route auto-closes it.
