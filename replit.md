...
- **Interactive Educational Content**:
    ...
    - **Physics of the Bicycle** (`/physics/bicycle`): bilingual blueprint-themed module under the Science → Physics dropdown as item #5 (after Waves). Deep-navy `#0b1733` background with cyan `#67e8f9` line-art and a faint blueprint grid. Three sections: (1) **Mystery of balance** — front-view technical drawing with a spinning gyro wheel (toggleable) + side-view caster/trail diagram showing the steering axis hitting the ground a few cm in front of the tire-contact point with a toggleable rose TRAIL measurement and self-steer arrow. Wheels are positioned so their tires visually touch the drawn ground line. Copy explicitly notes (citing Kooijman et al., *Science* 2011) that gyro+trail are *contributing* mechanisms, not the sole cause of self-stability. (2) **Four pillars** — clickable grid of Angular Momentum (L=I·ω), Centre of Mass (Σm·r/Σm), Friction (f=μN), Gearing & Torque (τ_out/τ_in=r_out/r_in); each opens a detail panel with formula + bilingual explanation + 3 real-world examples. (3) **Balance Lab** — inverted-pendulum simulation. Implementation pattern (carry forward): single `useEffect`/rAF loop reads `stateRef.current` so state changes don't restart the loop and don't create stale closures; `lastTimeRef` for `dt`; `visibilitychange` listener pauses the integrator when the tab is hidden. Equation: `θ̈ = (g/L)·sin θ − k·v²·sin θ − c·v·θ̇ + ξ(v)` with restoring effect ∝ v² and noise ∝ 1/v. Crash at |θ|>π/4 displays a "CRASH! Stayed up for X.X s" overlay. **Reset must call `setRunning(false)`** in addition to clearing angle/omega/fallen — otherwise at low speed the noise term immediately tips the bike again before the user can react. Speed thresholds: stable >8, wobbly 4–8, unstable <4 km/h, with Slow/Fast preset buttons. All technical terms bilingual EN/Khmer; all animations wrapped in `prefers-reduced-motion: reduce`; all SVGs have `role="img"` + bilingual `<title>/<desc>`.
